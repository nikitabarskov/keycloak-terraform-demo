...
#   1. Obtain a token from the mock-oauth2-server (external IdP)
#   2. Exchange it for a Keycloak token (external-to-internal)
#   3. Exchange the Keycloak token for the external IdP token (internal-to-external)
#
# Prerequisites:
#   - docker compose up -d
...
MOCK_OAUTH2_URL="${MOCK_OAUTH2_URL:-http://host.docker.internal:8090}"
REALM="${REALM:-demo}"
IDP_ALIAS="${IDP_ALIAS:-mock-oauth2-server}"
MOCK_ISSUER_ID="${MOCK_ISSUER_ID:-default}"
...
# -- Step 1: Get a token from mock-oauth2-server ------------------------------
header "Step 1: Obtain a token from mock-oauth2-server (external IdP)"

EXTERNAL_TOKEN_RESPONSE=$(curl -s -X POST \
	"${MOCK_OAUTH2_URL}/${MOCK_ISSUER_ID}/token" \
...
if [ -z "$EXTERNAL_TOKEN" ]; then
	...
	error "Failed to obtain token from mock-oauth2-server. Is it running on ${MOCK_OAUTH2_URL}?"
fi

info "External token obtained successfully."
info "External token payload:"
decode_jwt_payload "$EXTERNAL_TOKEN"

# -- Step 2: Exchange external token for a Keycloak token ---------------------
header "Step 2: Exchange external token for a Keycloak token (external-to-internal)"
...
# -- Step 3: Exchange Keycloak token for external token -----------------------
...
if [ -z "$EXCHANGED_TOKEN" ]; then
	...
	error "Failed to exchange Keycloak token for external token. See error above."
fi

info "External token retrieved via token exchange!"
info "Exchanged token payload:"
decode_jwt_payload "$EXCHANGED_TOKEN"

# -- Summary ------------------------------------------------------------------
header "Token Exchange Complete"
info "1. External token from mock-oauth2-server was exchanged for a Keycloak token"
info "2. The Keycloak token was then exchanged back for the external IdP token"
info "   using requested_issuer=${IDP_ALIAS}"
