...
The SPA never sees Keycloak tokens. It only holds:
  - A session cookie (hero_session) for Hero API calls
  - A short-lived DIPS/external token obtained via Option A or B
"""
...
MOCK_OAUTH2_URL = os.environ.get("MOCK_OAUTH2_URL", "http://host.docker.internal:8090")
...
app = FastAPI(title="Hero API - RFC-001 Demo")
...
def get_session(request: Request) -> dict | None:
    """Resolve session from the hero_session cookie."""
    cookie = request.cookies.get("hero_session")
    if not cookie:
        return None
    try:
...
async def index(request: Request):
    ...
    return templates.TemplateResponse(
        "index.html",
        {
            ...
            "mock_oauth2_url": MOCK_OAUTH2_URL,
        },
    )
...
async def oauth_logout(request: Request):
    ...
    response.delete_cookie("hero_session")
    return response
...
@app.get("/dips/token")
async def dips_token(request: Request):
    """
    Option A: Backend token exchange.

    Exchanges the Keycloak access token (from server-side session) for the
    stored external IdP token using requested_issuer. The external token is
    returned to the SPA for direct DIPS API calls.
