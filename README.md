...
A local Keycloak setup for testing the [keycloak/keycloak](https://registry.terraform.io/providers/keycloak/keycloak/latest) Terraform provider. Provisions a confidential client with client roles mapped to a custom token claim, authenticated via the Client Credentials grant.
...
| `keycloak_openid_user_client_role_protocol_mapper` | Maps client roles into a `backend-roles` claim in the access token |
